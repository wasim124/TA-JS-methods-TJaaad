...
console.log(
  peopelName.filter((name) => name.startsWith("J") || name.startsWith("P"))
);

// Log the length of filtered named of people in peopleName that starts with 'A' and 'C'

console.log(
  peopelName.filter((name) => name.startsWith("A") || name.startsWith("C"))
    .length
);
...
let averageGradeMale = persons
  .filter((person) => person.sex === "M")
  .map((person) => person.grade);
averageGradeMale.reduce((acc, cv) => {
  return acc + cv;
...
let averageGradeFemale = persons
  .filter((person) => person.sex === "F")
  .map((person) => person.grade);
averageGradeFemale.reduce((acc, cv) => {
  return acc + cv;
...
let nameJorP =persons.filter((name) => name.startsWith("J") || name.startsWith("P"));

let  gradeJorP = nameJorP.map((person) => person.grade);

[...gradeJorP].sort((a, b) => a - b).pop();
...
let descending=[...peopleGrade].sort((a, b) => b - a);
